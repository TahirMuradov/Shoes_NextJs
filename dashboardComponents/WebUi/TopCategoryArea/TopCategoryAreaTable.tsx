...
const TopCategoryAreaTable:React.FC<{Lang:Locale,page:number,apiDomen:string|undefined}>=({Lang,page,apiDomen})=>{
   ...
   if (res) {
    ...
    function Delete(id:string){
        ...
        SetLoader(true)
       fetch(`${apiDomen}api/TopCategoryArea/RemoveTopCategoryArea?Id=${id}`, {
          headers: {
            'Accept': 'application/json',
            'Content-Type': 'application/json',
            'langCode': `${Lang}`,  // You can dynamically set this value based on user selection or other logic
   ...
   'Authorization':`Bearer ${sessions.data?.user.token}`
      ...
      <Link href={`/dashboard/webui/topcategoryarea/edit/${row.id}`} className="m-3 bg-transparent hover:bg-yellow-500 text-yellow-700 font-semibold hover:text-white py-2 px-4 border border-yellow-500 hover:border-transparent rounded">
        Edit
      </Link>
                          </StyledTableCell>
                  </StyledTableRow>
...
export default TopCategoryAreaTable